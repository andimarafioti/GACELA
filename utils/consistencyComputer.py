...
def substractMeanAndDivideByStd(aDistribution):
    unmeaned = aDistribution - torch.mean(aDistribution, dim=(1, 2), keepdim=True)
    shiftedtt = unmeaned / torch.sqrt(torch.sum(torch.abs(unmeaned) ** 2, dim=(1, 2), keepdim=True))
    return shiftedtt
...
def consistency(spectrogram):
    ...
    consistencies = torch.sum(absttderiv * absffderiv, dim=(1, 2))
